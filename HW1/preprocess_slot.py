...
def main(args):
    ...
    for split in ["train", "eval"]:
        ...
        tags.update({tag for instance in dataset for tag in instance["tags"]}) #收集所有tags
        words.update([token for instance in dataset for token in instance["tokens"]]) #計數所有vocabulary出現的次數

    tag2idx = {tag: i for i, tag in enumerate(tags)} #tag轉index

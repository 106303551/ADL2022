...
for j in range(len(ds['test'])):
  idx=ds['test'][j]['label']
  cur_context=context[ds['test'][j]['paragraphs'][idx]]
  context_list.append(cur_context)

      
ds['test'] = ds['test'].add_column("context",context_list)
...
validation_features = ds["test"].map(
...
import numpy as np
#raw_predictions=2*example_num*512
final_predictions = postprocess_qa_predictions(ds["test"], validation_features, raw_predictions.predictions)
